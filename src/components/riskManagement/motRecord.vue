<template>
  <div>
    <div class="customer_list">
      <div class="search">
        <div>
          <span style="font-size:14px;">年检记录</span>
        </div>
        <div>
      ...
      >
        ...
        <el-table-column prop="motDay" label="年检日期"></el-table-column>
        <el-table-column prop="handlerName" label="年检处理人"></el-table-column>
        <el-table-column prop="motAmount" label="年检金额"></el-table-column>
        <el-table-column prop="nextDay" label="下次年检日期"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getMotRecord() {
      ...
      http.getMotList(params, tenantId).then(data => {
        console.log(data);
        if (data.code == 0) {
    ...
    gotoDetail(row) {
      this.$router.push({
        name: `addMot`,
        query: { pageType: "detail", motInfo: JSON.stringify(row) }
      });
    },
