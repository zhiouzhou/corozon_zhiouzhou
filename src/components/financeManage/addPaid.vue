  ...
  <div id="addCustomer">
    <div style="background:#fff">
      <div class="add_title">
        ...
        <p>{{pageType=='add'?'新增已付记录':'已付记录详情'}}</p>
      </div>
    </div>
      ...
      <el-row :gutter="60">
        <el-col :span="12" :xl="8">
          ...
          <el-form
            ...
            label-width="auto"
          >
            <el-form-item label="付款类型：" prop="recordType">
              <el-select
        ...
        <el-col :span="12">
          ...
          >
            ...
            <el-form-item label="已付日期：" prop="paidDate">
              <el-date-picker
                size="mini"
    ...
    <div class="checkForm">
      ...
      >
        ...
        <div class="addCheckInfo">
          <el-button type="primary" @click="addCheckInfo">新增已付信息</el-button>
        </div>
      </el-form>
    </div>
  </div>
  ...
  methods: {
    ...
    addMot() {
      ...
      this.$refs.acceptedInfo.validate(valid => {
        if (valid) {
          signHttp.addPaidRecrd(acceptedInfo, tenantId).then(data => {
            console.log(data);
            if (data.code == 0) {
              this.$message(`新增已付记录成功`);
              this.$router.back(-1);
            } else {
              this.$message(data.desc);
            }
