      ...
      <el-row :gutter="60">
        <el-col :span="12" :xl="8">
          ...
          >
            ...
            <el-form-item label="应退日期：" prop="arDate">
              <el-date-picker
                size="mini"
            ...
            <el-form-item label="实退金额：" prop="paidAmount">
              <el-input v-model="recordInfo.paidAmount" size="mini" placeholder="实退金额"></el-input>
            </el-form-item>
        ...
        <el-col :span="12">
          ...
          >
            <el-form-item label="缴纳状态：" prop="payRole">
              <el-radio v-model="recordInfo.paidStatus" label="0" disabled>未退</el-radio>
              <el-radio v-model="recordInfo.paidStatus" label="1">已退</el-radio>
            </el-form-item>
            ...
            <el-form-item style="margin-top:30px;" v-if="pageType=='add'">
              <el-button
                ...
                type="primary"
                @click="addMot"
                :disabled="contractInfo.leaseType=='DAY'"
              >确定</el-button>
  ...
  methods: {
    ...
    addMot() {
      ...
      this.$refs.recordInfo.validate(valid => {
        if (valid) {
          signHttp.returnDeposit(recordInfo, tenantId).then(data => {
            console.log(data);
            if (data.code == 0) {
              this.$message(`退押金成功`);
              this.$router.back(-1);
    ...
    handleSelect(item) {
      ...
      signHttp
        .getDepositPayable({ contractId: contractId }, tenantId)
        .then(data => {
          if (data.code == 0) {
            console.log(data.data);
            this.recordInfo.arDate = data.data.arDate ? data.data.arDate : ""; //应交日期
            this.recordInfo.arAmount = data.data.arAmount
              ? data.data.arAmount
              : this.contractInfo.deposit; //应缴金额
          } else {
            this.$message(data.desc);
          }
        });
    ...
    selectPay(value) {
      ...
      this.recordInfo.payer = roleInfo.name;
    },
    //删除年检记录
    deleteMot(row) {
