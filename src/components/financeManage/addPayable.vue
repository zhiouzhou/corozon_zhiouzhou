      ...
      <el-row :gutter="60">
        <el-col :span="12" :xl="8">
          ...
          <el-form
            ...
            label-width="auto"
          >
            <el-form-item label="应付类型：" prop="recordType">
              <el-select
                v-model="receivableInfo.recordType"
                size="small"
                placeholder="请选择应付类型"
            ...
            <el-form-item label="车牌号：" prop="licensePlate">
              <el-autocomplete
                v-model="receivableInfo.licensePlate"
            ...
            <el-form-item label="客户姓名：" prop="userName">
              <el-input
                v-model="receivableInfo.userName"
                class="brand_input"
                size="mini"
                placeholder="客户姓名"
                :disabled="pageType=='detail'?true:false"
        ...
        <el-col :span="12">
          ...
          >
            <el-form-item label="合同编号：">
              <el-input
                v-model="receivableInfo.contractNo"
                size="mini"
                placeholder="请输入合同编号"
                :disabled="pageType=='detail'?true:false"
              ></el-input>
            </el-form-item>
            <el-form-item label="应付日期：" prop="arDate">
              <el-date-picker
                size="mini"
                v-model="receivableInfo.arDate"
                type="date"
                placeholder="请选择应付日期"
            ...
            <el-form-item label="应付金额：" prop="arAmount">
              <el-input
                v-model="receivableInfo.arAmount"
                size="mini"
                placeholder="请输入应付金额"
                :disabled="pageType=='detail'?true:false"
              ></el-input>
            ...
            <el-form-item label="付款方角色：" prop="payRole">
              <el-radio v-model="receivableInfo.payRole" label="0">客户</el-radio>
              <el-radio v-model="receivableInfo.payRole" label="1">合作伙伴</el-radio>
            </el-form-item>
            <el-form-item label="备注：">
              <el-input
                type="textarea"
                v-model="receivableInfo.remarks"
                rows="5"
                size="mini"
                placeholder="请输入备注信息"
                :disabled="pageType=='detail'?true:false"
...
export default {
  ...
  activated() {
    ...
    if (this.$route.query.receivableInfo) {
      let receivableInfo = JSON.parse(this.$route.query.receivableInfo);
      this.receivableInfo = {
        id: receivableInfo.id,
        licensePlate: receivableInfo.licensePlate,
        brand: receivableInfo.brand,
        model: receivableInfo.model,
        handlerName: receivableInfo.handlerName,
        motDay: receivableInfo.motDay,
        motAmount: receivableInfo.motAmount,
        nextDay: receivableInfo.nextDay,
        remarks: receivableInfo.remarks
      };
      this.newMotInfo = {
        id: receivableInfo.id,
        licensePlate: receivableInfo.licensePlate,
  ...
  methods: {
    ...
    //查询品牌车型列表
    addMot() {
      let { receivableInfo, tenantId } = this;
      this.$refs.receivableInfo.validate(valid => {
        if (valid) {
          signHttp.addPayable(receivableInfo, tenantId).then(data => {
            console.log(data);
            if (data.code == 0) {
              this.$message(`新增应付成功`);
              this.$router.back(-1);
    ...
    querySearchAsync(queryString, cb) {
      ...
      let status = ["SIGNED"];
      // let params = { plateNumber: queryString, contractStatus: "SIGNED" };
      if (!queryString) {
        return;
      }
      let addVehicleList = [{}];
    ...
    handleSelect(item) {
      ...
      this.receivableInfo.licensePlate = licensePlate;
      this.receivableInfo.userName = item.customerName;
      this.receivableInfo.contractId = item.id;
      let that = this;
      http.getVehicleModel({ licensePlate }, tenantId).then(data => {
        console.log(data);
        if (data.code == 0) {
          that.receivableInfo.brand = data.data.brand;
          that.receivableInfo.model = data.data.model;
      ...
      signHttp.getContractDetail(tenantId, contractId).then(data => {
        ...
        if (data.code == 0) {
          ...
          this.receivableInfo.contractNo = data.data.contractNo;
        }
      });
    },
    clear() {
    ...
    //删除年检记录
    deleteMot(row) {
      let params = { id: receivableInfo.id };
      let tenantId = this.tenantId;
    ...
    diffDevinfo() {
      for (let k in this.newMotInfo) {
        if (this.newMotInfo[k] != this.receivableInfo[k]) {
          if (!this.diffData) {
            this.diffData = {};
          }
          this.diffData[k] = this.receivableInfo[k];
    ...
    editMot() {
      ...
      if (this.pageStatus == 1) {
        ...
        if (!this.diffData) {
          ...
          this.diffData.id = this.receivableInfo.id;
          this.diffData.licensePlate = this.receivableInfo.licensePlate;
        }
        let params = !this.diffData ? receivableInfo : this.diffData;
