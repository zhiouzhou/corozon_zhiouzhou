      ...
      <el-row :gutter="60">
        <el-col :span="12" :xl="8">
          ...
          >
            ...
            <el-form-item label="首次缴纳租金金额：" prop="paidAmount">
              <el-input
                v-model="recordInfo.paidAmount"
                size="mini"
                placeholder="请输入首次缴纳租金金额"
                :disabled="pageType=='detail'?true:false"
              ></el-input>
            </el-form-item>
            <el-form-item label="下次交租金日期：" prop="nextPaidDate" v-if="recordInfo.rentType==1">
              <el-date-picker
                size="mini"
                v-model="recordInfo.nextPaidDate"
                type="date"
                placeholder="请选择下次交租金日期"
                value-format="yyyy-MM-dd"
                :disabled="pageType=='detail'?true:false"
              ></el-date-picker>
            </el-form-item>
        ...
        <el-col :span="12">
          ...
          >
            ...
            <el-form-item label="押金缴纳金额：" prop="depositAmount">
              <el-input
                v-model="recordInfo.depositAmount"
                size="mini"
                placeholder="请输入押金缴纳金额"
                :disabled="pageType=='detail'?true:false"
              ></el-input>
            </el-form-item>
            <el-form-item label="管理费缴纳金额：" prop="manageCost">
              <el-input
                v-model="recordInfo.manageCost"
                size="mini"
                placeholder="请输入管理费缴纳金额"
                :disabled="pageType=='detail'?true:false"
              ></el-input>
            </el-form-item>
            <el-form-item label="基础保险费：" prop="basicPremium">
              <el-input
                v-model="recordInfo.basicPremium"
                size="mini"
                placeholder="请输入基础保险费"
                :disabled="pageType=='detail'?true:false"
              ></el-input>
            </el-form-item>
            <el-form-item label="不计免赔保险费：" prop="excludeDutyPremium">
              <el-input
                v-model="recordInfo.excludeDutyPremium"
                size="mini"
                placeholder="请输入不计免赔保险费"
                :disabled="pageType=='detail'?true:false"
              ></el-input>
            </el-form-item>
  ...
  methods: {
    ...
    addMot() {
      ...
      this.$refs.recordInfo.validate(valid => {
        if (valid) {
          signHttp.addSignReceipts(recordInfo, tenantId).then(data => {
            console.log(data);
            if (data.code == 0) {
              this.$message(`签约收款成功`);
              this.$router.back(-1);
    ...
    handleSelect(item) {
      ...
      http.getVehicleModel({ licensePlate }, tenantId).then(data => {
        console.log(data);
        if (data.code == 0) {
          that.recordInfo.brand = data.data.brand;
          that.recordInfo.model = data.data.model;
          this.brandModel = data.data.brand + "-" + data.data.model;
        }
      });
      signHttp.getContractDetail(tenantId, contractId).then(data => {
        ...
        if (data.code == 0) {
          ...
          this.recordInfo.paidAmount = data.data.rent;
          this.recordInfo.depositAmount = data.data.deposit;
          this.recordInfo.manageCost = data.data.managementFee;
          this.recordInfo.basicPremium = data.data.basicPremium;
          this.recordInfo.excludeDutyPremium = data.data.aer;
          this.recordInfo.contractEndDate = data.data.endTime.split(" ")[0];
          this.recordInfo.userId = data.data.customer.customerId;
          this.recordInfo.contractNo = data.data.contractNo;
          this.recordInfo.rentType = data.data.leaseType == "MONTH" ? 1 : 0;
        }
      });
    },
    clear() {
