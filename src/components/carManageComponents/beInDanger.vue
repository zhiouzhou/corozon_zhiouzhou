  ...
  <div>
    <div class="customer_list">
      ...
      >
        ...
        <el-table-column prop="lossOccurredTime" label="出险时间"></el-table-column>
        <el-table-column prop="responseParty" label="责任方"></el-table-column>
        <el-table-column label="理赔状态">
          <template slot-scope="scope">
            <span>{{scope.row.claimStatus==0?'处理中':'已完成'}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="ownAmount" label="己方金额"></el-table-column>
        <el-table-column prop="thirdAmount" label="第三方金额"></el-table-column>
        <el-table-column prop="lossOccurredDesc" label="出险描述"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getMotRecord(plate) {
      ...
      http.getBeInDangerRecord(params, tenantId).then(data => {
        console.log(data);
        if (data.code == 0) {
